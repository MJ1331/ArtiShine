...
llm = ChatOpenAI(
    ...
    default_headers={
        ...
        "X-Title": "Artisan Onboarding API"
    }
)
...
def get_welcome_caption(artisan_details: str) -> Dict:
    """
    Generates a social media caption. Input must be a comma-separated string:
    name, YYYY-MM-DD, shop_name, location
    """
    try:
...
def generate_and_upload_onboarding_image(artisan_details_with_userid: str) -> str:
    """
    Generates and uploads an onboarding image. Input must be a comma-separated string:
    UserID, name, YYYY-MM-DD, shop_name, location
    """
...
# --- API Endpoint ---
@app.post("/generate-post", response_model=OnboardingOutput)
async def create_onboarding_post(details: ArtisanID):
    ...
    try:
        ...
        missing_fields = [field for field in required_fields if field not in artisan_data]
        if missing_fields:
            raise HTTPException(
                status_code=400, 
                detail=f"Artisan document '{details.UserID}' is missing required fields: {', '.join(missing_fields)}"
            )
        ...
        dob_string_from_db = artisan_data['date_of_birth']
        dob_obj = datetime.datetime.strptime(dob_string_from_db, '%d-%m-%Y')
    ...
    try:
        ...
        if not caption_data:
            raise HTTPException(status_code=500, detail=f"Caption generation failed. Last agent observation was: {response.get('intermediate_steps', [])[-1][1] if response.get('intermediate_steps') else 'None'}")
        ...
        if not user_doc.exists or 'onboardingImageUrl' not in user_doc.to_dict():
            raise HTTPException(status_code=404, detail="Image URL not found in Firestore. The image tool may have failed.")
